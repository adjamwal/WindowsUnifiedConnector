...
std::string CheckinManifestRetriever::GetCheckinManifestFrom( std::string uri )
{
    ...
    std::string response;
    int32_t respStatus = m_cloud.Checkin( "{}", response );

    if( respStatus != 200 )
    {
        throw std::exception( __FUNCTION__ ": HttpPost error status %d, uri:%s", respStatus, uri.c_str() )
    }
