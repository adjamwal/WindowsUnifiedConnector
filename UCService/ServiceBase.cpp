...
{
    LOG_DEBUG( L"Service trying to start." );
    try
    {
    ...
    {
        LOG_ERROR( L"Service failed to start, error %d", dwError );
        SetServiceStatus( SERVICE_STOPPED, dwError );
    }
    catch( ... )
    {
        LOG_ERROR( L"Service failed to start." );
...
{
    LOG_DEBUG( L"Service trying to stop." );
    ...
    {
        LOG_ERROR( L"Service failed to stop, error %d", dwError );
        SetServiceStatus( dwOriginalState );
    }
    catch( ... )
    {
        LOG_ERROR( L"Service failed to stop." );
        SetServiceStatus( dwOriginalState );
    }
}
...
{
    LOG_DEBUG( L"Service trying to pause." );
    try
    {
    ...
    {
        LOG_ERROR( L"Service failed to pause, error %d", dwError );
        SetServiceStatus( SERVICE_RUNNING );
    }
    catch( ... )
    {
        LOG_ERROR( L"Service failed to pause." );
        SetServiceStatus( SERVICE_RUNNING );
    }
}
...
{
    LOG_DEBUG( L"Service trying to continue." );
    try
    {
    ...
    {
        LOG_ERROR( L"Service failed to continue, error %d", dwError );
        SetServiceStatus( SERVICE_PAUSED );
    }
    catch( ... )
    {
        LOG_ERROR( L"Service failed to continue." );
        SetServiceStatus( SERVICE_PAUSED );
    }
}
...
{
    LOG_DEBUG( L"Service trying to shut down." );
    try
    {
        OnShutdown();
        SetServiceStatus( SERVICE_STOPPED );
    ...
    {
        LOG_ERROR( L"Service failed to shutdown, error %d", dwError );
    }
    catch( ... )
    {
        LOG_ERROR( L"Service failed to shut down." );
    }
}
...
void ServiceBase::InitializeLogging( BOOL fCanStop, BOOL fCanShutdown, BOOL fCanPauseContinue )
{
    ...
    {
        m_etwRegHandle = 0;
        LOG_ERROR( L"EventRegister failed, provider not registered." );
    }
    else
    {
